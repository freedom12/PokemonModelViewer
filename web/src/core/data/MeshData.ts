...
export class MeshData implements IMeshData {
  ...
  toBufferGeometry(): THREE.BufferGeometry {
    ...
    // 设置第二套 UV 坐标
    if (this.uvs2) {
      geometry.setAttribute('uv2', new THREE.BufferAttribute(this.uvs2, 2));
    }
    ...
    // 设置蒙皮索引
    if (this.skinIndices) {
      geometry.setAttribute('skinIndex', new THREE.BufferAttribute(this.skinIndices, 4));
    }

    // 设置蒙皮权重
    if (this.skinWeights) {
      geometry.setAttribute('skinWeight', new THREE.BufferAttribute(this.skinWeights, 4));
    }
  ...
  validate(): { valid: boolean; errors: string[] } {
    ...
    if (this.normals) {
      ...
      if (
        this.normals.length !== expectedLength3 &&
        this.normals.length !== expectedLength4
      ) {
        errors.push(
          `Normals data length mismatch: expected ${expectedLength3} or ${expectedLength4}, got ${this.normals.length}`
        );
      }
    }
    ...
    if (this.uvs && this.uvs.length !== this.vertexCount * 2) {
      errors.push(
        `UVs data length mismatch: expected ${this.vertexCount * 2}, got ${this.uvs.length}`
      );
    }
    ...
    // 检查蒙皮数据一致性
    if (
      (this.skinIndices && !this.skinWeights) ||
      (!this.skinIndices && this.skinWeights)
    ) {
    ...
    if (this.skinIndices && this.skinIndices.length !== this.vertexCount * 4) {
      errors.push(
        `Skin indices data length mismatch: expected ${this.vertexCount * 4}, got ${this.skinIndices.length}`
      );
    }

    if (this.skinWeights && this.skinWeights.length !== this.vertexCount * 4) {
      errors.push(
        `Skin weights data length mismatch: expected ${this.vertexCount * 4}, got ${this.skinWeights.length}`
      );
    }
