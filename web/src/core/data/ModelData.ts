...
export class ModelData implements IModelData {
  ...
  private static parseSingleMesh(
    ...
    // 使用 meshShapeName 作为网格名称，如果没有则使用索引
    const meshName = meshShape.meshShapeName() || `mesh_${meshIndex}`;
      ...
      if (shader) {
        ...
        if (sn) {
          ...
          for (let j = 0; j < valuesCount; j++) {
            ...
            if (value) {
              const valueName = value.stringName();
              const valueStr = value.stringValue();
              if (valueName) {
