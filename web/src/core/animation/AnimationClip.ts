  ...
  constructor(data: IAnimationData) {
    ...
    for (const track of data.tracks) {
      if (track.type === 'bone') {
        ...
        const visibilityTrack = VisibilityAnimationTrack.fromData(
          track as VisibilityTrackData
        );
  ...
  static fromTRANM(tranm: TRANM, name: string = 'bone_animation'): AnimationClip {
    ...
    if (boneAnimation) {
      ...
      for (let i = 0; i < tracksLength; i++) {
        ...
        // 解析缩放轨道
        const scaleTrack = AnimationClip.parseVectorTrack(boneTrack.scaleType(), (obj) =>
          boneTrack.scale(obj)
        );
