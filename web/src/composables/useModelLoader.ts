...
> = {
  ...
  [LoadingStage.LOADING_FILES]: {
    start: 0,
    end: 30,
    message: "加载模型文件...",
  },
  [LoadingStage.PARSING_DATA]: {
    start: 30,
    end: 45,
    message: "解析模型数据...",
  },
  [LoadingStage.CREATING_GEOMETRY]: {
    start: 45,
    end: 60,
    message: "创建几何体...",
  },
  [LoadingStage.LOADING_TEXTURES]: {
    start: 60,
    end: 90,
    message: "加载纹理贴图...",
  },
  [LoadingStage.CREATING_MESH]: {
    start: 90,
    end: 100,
    message: "创建网格对象...",
  },
...
export function useModelLoader(
  ...
  function updateProgress(
    ...
    const totalProgress =
      stageInfo.start +
      (stageInfo.end - stageInfo.start) * Math.min(stageProgress, 1);
      ...
      for (let meshIdx = 0; meshIdx < totalMeshes; meshIdx++) {
        ...
        for (const group of groups) {
          ...
          if (materialName && modelData.trmtr) {
            ...
            const trmtrMaterial = findMaterialByName(
              modelData.trmtr,
              materialName,
            );
        ...
        // 创建网格
        const mesh = createMeshWithMaterials(
          geometry,
          groups,
          meshMaterials,
          skeleton,
