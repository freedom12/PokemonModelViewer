...
export function useThreeScene(
   ...
   * 显示/隐藏顶点法线
   */
  function setVertexNormalsVisible(
    visible: boolean,
          ...
          const vertices = [
            {
              ...
              uv: uvAttribute
                ? getVector2FromAttribute(uvAttribute, a)
                : undefined,
            },
            {
              ...
              uv: uvAttribute
                ? getVector2FromAttribute(uvAttribute, b)
                : undefined,
            },
            {
              ...
              uv: uvAttribute
                ? getVector2FromAttribute(uvAttribute, c)
                : undefined,
            },
          ];
  ...
  function highlightSelectedTriangle(
    ...
    if (state.selectedTriangleHighlight) {
      ...
      if (Array.isArray(state.selectedTriangleHighlight.material)) {
        state.selectedTriangleHighlight.material.forEach((material) =>
          material.dispose(),
        );
      } else {
    ...
    // 获取相机位置
    const cameraPosition = state.camera
      ? state.camera.position.clone()
      : new THREE.Vector3(0, 0, 1);

    // 计算从三角形中心到相机的向量
    const toCamera = new THREE.Vector3()
      .subVectors(cameraPosition, worldCenter)
      .normalize();
  ...
  function highlightSelectedBone(boneIndex: number | null): void {
    ...
    if (state.selectedBoneHighlight) {
      ...
      if (Array.isArray(state.selectedBoneHighlight.material)) {
        state.selectedBoneHighlight.material.forEach((material) =>
          material.dispose(),
        );
      } else {
    ...
    transformNodes.forEach((node, rigIdx) => {
      ...
      if (parentIdx >= 0) {
        ...
        if (parentJoint && childJoint) {
          ...
          // 创建骨骼线段
          const boneGeometry = new THREE.BufferGeometry().setFromPoints([
            parentWorldPos,
            childWorldPos,
          ]);
